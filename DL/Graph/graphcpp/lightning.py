...
class GraphCPPKFoldDataModule(LightningDataModule):
    ...
    def setup(self, stage=None):
        if not self.data_train and not self.data_val:
            dataset_train = CPPDataset(
                root=self.hparams.folder, _split="train", fp_type=self.hparams.fp_type
            ).shuffle()
            dataset_val = CPPDataset(
                root=self.hparams.folder, _split="val", fp_type=self.hparams.fp_type
            ).shuffle()
...
class GraphCPPModule(L.LightningModule):
    ...
    def forward(self, data):
        ...
        # The model returns (pred, label, inbetween_embedding)
