...
from graphcpp.generalconv import (
    GeneralConvLayer,
    GeneralEdgeConvLayer as GraphGymEdgeConvLayer,
)
...
class GeneralLayer(torch.nn.Module):
    ...
    def __init__(
        self, name, dim_in, dim_out, has_act=True, dropout=0.0, act="relu", **kwargs
    ):
        super(GeneralLayer, self).__init__()
        self.has_l2norm = kwargs.get("has_l2norm", False)
        self.dropout = dropout
        self.layer = layer_dict[name](
            dim_in, dim_out, bias=not kwargs.get("has_bn", False), **kwargs
        )
        layer_wrapper = []
    ...
    def forward(self, batch):
        ...
        if isinstance(batch, torch.Tensor):
            ...
            if self.has_l2norm:
                batch = F.normalize(batch, p=2.0, dim=1)
        else:
...
class MLP(torch.nn.Module):
    def __init__(
        self, dim_in, dim_out, bias=True, dim_inner=None, num_layers=2, **kwargs
    ):
...
class GNNStackStage(torch.nn.Module):
    def __init__(self, dim_in, dim_out, num_layers, stage_type, layer_type, **kwargs):
        ...
        for i in range(num_layers):
            ...
            layer = GeneralLayer(
                name=layer_type, dim_in=d_in, dim_out=dim_out, has_act=True, **kwargs
            )
...
class GNNGraphHead(torch.nn.Module):
    def __init__(self, dim_in, dim_out, num_layers, pooling="add", **kwargs):
        ...
        self.layer_post_mp = MLP(
            dim_in=dim_in * (2 if self.layer_fingerprints > 0 else 1),
            dim_out=dim_out,
            num_layers=num_layers,
            bias=True,
            **kwargs
        )  # Times two because of concat of fp and graph embedding

        if self.layer_fingerprints > 0:
            self.fingerprint = MLP(
                dim_in=2048,
                dim_out=dim_in,
                num_layers=self.layer_fingerprints,
                bias=True,
            )  # 2048 because Morgan fingerprints
...
class SAGEConv(torch.nn.Module):
    def __init__(self, dim_in, dim_out, bias=False, conv_aggr="mean", **kwargs):
        ...
        self.model = pyg.nn.SAGEConv(
            in_channels=dim_in, out_channels=dim_out, bias=bias, aggr=conv_aggr
        )
...
class GATConv(torch.nn.Module):
    def __init__(self, dim_in, dim_out, bias=False, conv_dropout=0.0, **kwargs):
        ...
        self.model = pyg.nn.GATConv(
            dim_in, dim_out, bias=bias, edge_dim=11, dropout=conv_dropout
        )
...
class TransformerConv(torch.nn.Module):
    def __init__(
        self, dim_in, dim_out, bias=False, conv_dropout=0.0, n_heads=1, **kwargs
    ):
        super(TransformerConv, self).__init__()
        self.model = Sequential(
            [
                pyg.nn.TransformerConv(
                    in_channels=dim_in,
...
class CustomGeneralConv(torch.nn.Module):
    def __init__(self, dim_in, dim_out, bias=False, **kwargs):
        ...
        self.model = GeneralConvLayer(
            in_channels=dim_in, out_channels=dim_out, bias=bias
        )
...
class GeneralEdgeConv(torch.nn.Module):
    def __init__(self, dim_in, dim_out, bias=False, conv_aggr="sum", **kwargs):
        ...
        self.model = GraphGymEdgeConvLayer(
            dim_in, dim_out, bias=bias, edge_dim=11, aggr=conv_aggr
        )
