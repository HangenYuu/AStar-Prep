...
def run_one_training(experiment_name, dataset, max_epochs=100, **model_kwargs):
    ...
    module = GraphCPPDataModule(
        folder=dataset, fp_type=model_kwargs["fingerprint_type"]
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--experiment_name", type=str, help="Experiment name", default=f"{start_time}"
    )
    # dataset path folder
    parser.add_argument("--dataset", type=str, help="Dataset folder", default="data")
    parser.add_argument(
    ...
    result = run_one_training(
        args.experiment_name, args.dataset, args.epochs, **BEST_PARAMETERS
    )
